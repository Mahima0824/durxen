                  ...
                  <Card.Body className="py-3 px-4">
                    <div className="d-flex flex-wrap justify-content-between align-items-start gap-2">
                      ...
                      <div className="d-flex align-items-center">
                        <div
                          className={`avatar avatar-md bg-gradient-${stat.color} text-white rounded-3 me-3 d-flex align-items-center justify-content-center`}
                        >
            ...
            <Col xl={8}>
              <Card>
                <Card.Body>
                  ...
                  <div style={{ height: '345px' }} dir='ltr'>
                    <ResponsiveContainer width="100%" height="100%">
                      ...
                      <div key={index} className="border-0 py-2">
                        <div className="d-flex align-items-center">
                          <div className="position-relative me-3">
                            ...
                            {member.online && (
                              <span className="position-absolute bottom-0 end-0 bg-success rounded-circle border border-2 border-secondary-subtle" style={{ width: '12px', height: '12px' }}></span>
                            )}
                          </div>
                          <div className="flex-grow-1">
                            <h6 className="mb-0 text-dark">{member.name}</h6>
                    ...
                    <tbody>
                      {recentActivities.map((activity, index) => (
                        <tr key={index}>
                          <td>
                            <div className="d-flex align-items-center">
                              <div className="icon-shape icon-sm bg-soft-primary rounded-3 me-3">
                                <i
                                  className={`bi bi-${activity.icon} text-primary`}
                                ></i>
                              </div>
                              <div>
                          ...
                          <td>
                            <div className="avatar-group border-0">
                              {activity.assignedTo.map((id, i) => {
                                ...
                                return user ? (
                                  <div
                                    key={i}
                                    className="avatar avatar-sm text-primary border-0 me-1 bg-soft-primary rounded-circle"
                                    title={user.name}
                                  >
                                    {user.avatar}
                                  </div>
                ...
                <Card.Body>
                        ...
                        <defs>
                          {resourceAllocationData.map((entry, index) => (
                            ...
                            >
                              <stop
                                offset="0%"
                                stopColor={colors[gradientColor[entry.name]]}
                                stopOpacity="0.8"
                              />
                              <stop
                                offset="50%"
                  ...
                  <div className="mt-3">
                    {resourceAllocationData.map((item, index) => (
                      <div
                        key={index}
                        className={`d-flex justify-content-between align-items-center ${index !== resourceAllocationData.length - 1 ? "mb-3" : "mb-0"}`}
                      >
                        <div className="d-flex align-items-center">
                          <span
                            className={`badge-dot me-2 p-1 rounded-circle bg-${item.color}`}
                          ></span>
              ...
              <Card>
                <Card.Body>
                  ...
                  {performanceMetrics.map((metric, index) => (
                    <div
                      key={index}
                      className={`${index !== performanceMetrics.length - 1 ? "mb-3 pb-3 border-bottom" : ""}`}
                    >
                      <div className="d-flex justify-content-between align-items-center mb-1">
                        ...
                        <span
                          className={`badge bg-soft-${metric.trend === "up" ? "success" : "danger"} text-${metric.trend === "up" ? "success" : "danger"}`}
                        >
                          <i
                            className={`bi bi-arrow-${metric.trend === "up" ? "up" : "down"}`}
                          ></i>
                        </span>
                      </div>
                      ...
                      <div key={index} className={index === 0 ? "pb-2" : index === projectTimeline.length - 1 ? "pt-3" : "pt-3 pb-2"}>
                        <div className="d-flex">
                          <div className="me-3 text-center" style={{ minWidth: "24px" }}>
                            ...
                            {index < projectTimeline.length - 1 && <div className="vr h-75 my-1 opacity-25"></div>}
                          </div>
                          <div className="flex-grow-1">
                            <div className="d-block d-sm-flex justify-content-between align-items-center">
                              <h6 className="mb-0 text-dark">{item.title}</h6>
                              <small className="text-muted text-sm-end">{item.date}</small>
                            </div>
                            <div className="d-flex justify-content-between align-items-center mt-1">
                              <small className="text-muted">{item.completed} of {item.tasks} tasks</small>
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    <ListGroup variant="flush">
                      {teamMembers.slice(0, 3).map((member, index) => (
                        <ListGroup.Item key={index} className="border-0 bg-light">
                          <div className="d-flex align-items-center">
                            <div className="avatar avatar-sm avatar-title rounded-circle bg-soft-primary text-primary me-3">{member.name.charAt(0)}</div>
                            <div className="flex-grow-1">
                            ...
                            <div className="text-end">
                              ...
                              <small className="text-success"><i className="ri-arrow-up-line align-middle"></i> {Math.floor(Math.random() * 15) + 5}%</small>
                            </div>
                          </div>
                        </ListGroup.Item>
                      ))}
                    ...
                    <div className="d-flex justify-content-between align-items-center mb-2">
                      ...
                      <div className="avatar-group">
                        {teamMembers.slice(0, 4).map((member, index) => (
                          <div key={index} className="avatar border-0 avatar-sm bg-soft-primary me-1 rounded-circle text-primary" title={member.name}>{member.name.charAt(0)}</div>
                        ))}
                        {teamMembers.length > 4 && (
                          <div className="avatar avatar-xs rounded-circle bg-white border-0">
                            <span className="avatar-title text-muted">+{teamMembers.length - 4}</span>
                          </div>
                        )}
                      </div>
                    </div>
              ...
              <Card>
                <Card.Body>
                  <div className="d-flex flex-wrap gap-2 align-items-center justify-content-between mb-4">
                    ...
                    <ButtonGroup dir="ltr" size="xs" role="group">
                      <Button variant="outline-secondary" size="xs" active>
                        Monthly
                      </Button>
                  ...
                  <div style={{ height: "410px" }} dir="ltr">
                    <ResponsiveContainer width="100%" height="100%">
